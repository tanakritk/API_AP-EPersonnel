...
@Injectable()
export class LeaveApproveRepo {
  constructor(
    @InjectRepository(LeaveApprove)
    private readonly repo: Repository<LeaveApprove>,
  ) {}
  ...
  async getItemById(id: number) {
    try {
      return await this.repo.findOne({
        where: { id },
        relations: ['leave', 'mas_statusleave'],
      });
    } catch (err) {
  ...
  async findData(body: any) {
    try {
      ...
      const [data, count] = await this.repo.findAndCount({
        relations: relationValue || ['leave', 'mas_statusleave'],
